...
Fine-mapping was done on several cancer types. Prior weights were obtained from TORUS. I selected all LD blocks that have an FDR $\leq 10$% based on the q-values from TORUS. I used `L = 2` for fine-mapping with `susie_rss()` which works on out-of-sample LD.
...
sumstats <- as_tibble(data.table::fread('../../SUMMARY_STATISTICS/susie_ready/BRCA_Sumstats_SuSiE_Ready.txt.gz', sep = '\t', header = T))
...
top.snps <- merged.sumstats[merged.sumstats$susie_pip > 0.05 & merged.sumstats$CS == 1, ]
...
annot.names <- colnames(top.snps)[16:ncol(top.snps)]
...
#### Annotated Table (Immune only)

```{r}
kable(top.snps.kable[top.snps.kable$Annotation=='Immune_OCRs\nImmune_h3k27ac', ]) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
  scroll_box(width = "100%", height = "600px")
```
